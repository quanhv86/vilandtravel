...
namespace Nop.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
            ...
            if (product.HasDiscountsApplied)
            {
                ...
                foreach (var discount in product.AppliedDiscounts)
                {
                    if (_discountService.ValidateDiscount(discount, customer).IsValid &&
                        discount.DiscountType == DiscountType.AssignedToProductCodes)
        ...
        /// <summary>
        /// Gets allowed discounts applied to destinations
        /// </summary>
        /// <param name="product">Product</param>
        /// <param name="customer">Customer</param>
        /// <returns>Discounts</returns>
        protected virtual IList<DiscountForCaching> GetAllowedDiscountsAppliedToDestinations(Product product, Customer customer)
        {
            var allowedDiscounts = new List<DiscountForCaching>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;

            foreach (var discount in _discountService.GetAllDiscountsForCaching(DiscountType.AssignedToDestinations))
            {
                //load identifier of destinations with this discount applied to
                var discountDestinationIds = _discountService.GetAppliedDestinationIds(discount, customer);

                //compare with destinations of this product
                var productDestinationIds = new List<int>();
                if (discountDestinationIds.Any())
                {
                    //load identifier of destinations of this product
                    var cacheKey = string.Format(PriceCacheEventConsumer.PRODUCT_DESTINATION_IDS_MODEL_KEY,
                        product.Id,
                        string.Join(",", customer.GetCustomerRoleIds()),
                        _storeContext.CurrentStore.Id);
                    productDestinationIds = _cacheManager.Get(cacheKey, () =>
                        _destinationService
                        .GetProductDestinationsByProductId(product.Id)
                        .Select(x => x.DestinationId)
                        .ToList());
                }

                foreach (var destinationId in productDestinationIds)
                {
                    if (discountDestinationIds.Contains(destinationId))
                    {
                        if (_discountService.ValidateDiscount(discount, customer).IsValid &&
                            !allowedDiscounts.ContainsDiscount(discount))
                            allowedDiscounts.Add(discount);
        ...
        protected virtual IList<DiscountForCaching> GetAllowedDiscounts(Product product, Customer customer)
        {
            ...
            //discounts applied to destinations
            foreach (var discount in GetAllowedDiscountsAppliedToDestinations(product, customer))
                if (!allowedDiscounts.ContainsDiscount(discount))
                    allowedDiscounts.Add(discount);
        ...
        /// <param name="includeDiscounts">A value indicating whether include discounts or not for final price computation</param>
        /// <param name="quantity">Shopping cart item quantity</param>
        /// <param name="departureDate">Rental period start date (for rental products)</param>
            ...
            out List<DiscountForCaching> appliedDiscounts)
        {
            return GetFinalPrice(product, customer, null, additionalCharge, includeDiscounts, quantity,
                departureDate, rentalEndDate, out discountAmount, out appliedDiscounts);
        }
        /// <summary>
        /// Gets the final price
        /// </summary>
            ...
            out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            var cachedPrice = _cacheManager.Get(cacheKey, cacheTime, () =>
            {
                ...
                //rental products
                if (product.IsRental)
                    if (departureDate.HasValue && rentalEndDate.HasValue)
                        price = price * product.GetRentalPeriods(departureDate.Value, rentalEndDate.Value);
            ...
            out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            return GetUnitPrice(shoppingCartItem.Product,
                ...
                shoppingCartItem.DepartureDateUtc,
        ...
        /// <param name="departureDate">Rental start date (null for not rental products)</param>
        ...
        public virtual decimal GetUnitPrice(Product product,
            ...
            DateTime? departureDate, DateTime? rentalEndDate,
            bool includeDiscounts,
            out decimal discountAmount,
            out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            if (combination != null && combination.OverriddenPrice.HasValue)
            {
                finalPrice = GetFinalPrice(product,
                        ...
                        includeDiscounts,
                        quantity,
                        product.IsRental ? departureDate : null,
