...
namespace Nop.Web.Factories
{
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual IList<DestinationBriefInfoModel> PrepareProductDestinationModels(Product product)
        {
            if (product == null)
                throw new ArgumentNullException("product");

            string destinationsCacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_DESTINATIONS_MODEL_KEY,
                     product.Id,
                     _workContext.WorkingLanguage.Id,
                     string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                     _storeContext.CurrentStore.Id);
            var model = _cacheManager.Get(destinationsCacheKey,
                () => _destinationService.GetProductDestinationsByProductId(product.Id)
                    .Select(pm =>
                    {
                        var destination = pm.Destination;
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                SeName = product.GetSeName(),
                ProductType = product.ProductType,
                ShowSku = _catalogSettings.ShowProductCodeOnProductDetailsPage,
                Sku = product.Sku,
                ShowDestinationPartNumber = _catalogSettings.ShowDestinationPartNumber,
                FreeShippingNotificationEnabled = _catalogSettings.ShowFreeShippingNotification,
                DestinationPartNumber = product.DestinationPartNumber,
           ...
           //pictures
            ...
            if (!isAssociatedProduct)
            {
                model.ProductDestinations = PrepareProductDestinationModels(product);
            }
            ...
            if (product.IsRental)
            {
                ...
                if (updatecartitem != null)
                {
                    model.RentalStartDate = updatecartitem.DepartureDateUtc;
