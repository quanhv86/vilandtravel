...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        [NonAction]
        protected virtual void PrepareDestinationMappingModel(ProductModel model, Product product, bool excludeProperties)
        {
            if (model == null)
                throw new ArgumentNullException("model");

            if (!excludeProperties && product != null)
                model.SelectedDestinationIds = _destinationService.GetProductDestinationsByProductId(product.Id, true).Select(c => c.DestinationId).ToList();

            var allDestinations = SelectListHelper.GetDestinationList(_destinationService, _cacheManager, true);
            foreach (var m in allDestinations)
            {
                m.Selected = model.SelectedDestinationIds.Contains(int.Parse(m.Value));
                model.AvailableDestinations.Add(m);
            }
        }

        [NonAction]
        protected virtual void SaveDestinationMappings(Product product, ProductModel model)
        {
            var existingProductDestinations = _destinationService.GetProductDestinationsByProductId(product.Id, true);

            //delete destinations
            foreach (var existingProductDestination in existingProductDestinations)
                if (!model.SelectedDestinationIds.Contains(existingProductDestination.DestinationId))
                    _destinationService.DeleteProductDestination(existingProductDestination);

            //add destinations
            foreach (var destinationId in model.SelectedDestinationIds)
                if (existingProductDestinations.FindProductDestination(product.Id, destinationId) == null)
                {
                    //find next display order
                    var displayOrder = 1;
                    var existingDestinationMapping = _destinationService.GetProductDestinationsByDestinationId(destinationId, showHidden: true);
                    if (existingDestinationMapping.Any())
                        displayOrder = existingDestinationMapping.Max(x => x.DisplayOrder) + 1;
                    _destinationService.InsertProductDestination(new ProductDestination()
                    {
                        ProductId = product.Id,
        ...
        protected virtual void PrepareDiscountMappingModel(ProductModel model, Product product, bool excludeProperties)
        {
            ...
            foreach (var discount in _discountService.GetAllDiscounts(DiscountType.AssignedToProductCodes, showHidden: true))
            {
        ...
        {
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToProductCodes, showHidden: true);
        ...
        [FormValueRequired("go-to-product-by-product-code")]
        public virtual ActionResult GoToProductCode(ProductListModel model)
        {
            string productCode = model.GoDirectlyToProductCode;
            ...
            if (product == null)
            {
                var combination = _productAttributeService.GetProductAttributeCombinationByProductCode(productCode);
                if (combination != null)
                {
        ...
        public virtual ActionResult Create()
        {
            ...
            PrepareDestinationMappingModel(model, null, false);
        ...
        public virtual ActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //destinations
                SaveDestinationMappings(product, model);
                //ACL (customer roles)
            ...
            PrepareDestinationMappingModel(model, null, true);
        ...
        public virtual ActionResult Edit(int id)
        {
            ...
            PrepareDestinationMappingModel(model, product, false);
        ...
        public virtual ActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            PrepareDestinationMappingModel(model, product, true);
            ...
            var combinationsModel = combinations
                .Select(x =>
                {
                    var pacModel = new ProductModel.ProductAttributeCombinationModel
                    {
                        ...
                        Sku = x.Sku,
                        DestinationPartNumber = x.DestinationPartNumber,
                        Gtin = x.Gtin,
        ...
        public virtual ActionResult ProductAttributeCombinationUpdate(ProductModel.ProductAttributeCombinationModel model)
        {
            ...
            combination.DestinationPartNumber = model.DestinationPartNumber;
            ...
            AddProductAttributeCombinationModel model, FormCollection form)
        {
            ...
            if (!warnings.Any())
            {
                ...
                var combination = new ProductAttributeCombination
                {
                    ...
                    Sku = model.Sku,
                    DestinationPartNumber = model.DestinationPartNumber,
                    Gtin = model.Gtin,
