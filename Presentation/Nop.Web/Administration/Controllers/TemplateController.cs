...
namespace Nop.Admin.Controllers
{
    public partial class TemplateController : BaseAdminController
    {
        ...
        #region Destination templates

        public virtual ActionResult DestinationTemplates()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();
        ...
        [HttpPost]
        public virtual ActionResult DestinationTemplateUpdate(DestinationTemplateModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();
            ...
            var template = _destinationTemplateService.GetDestinationTemplateById(model.Id);
            if (template == null)
                throw new ArgumentException("No template found with the specified id");
            template = model.ToEntity(template);
            _destinationTemplateService.UpdateDestinationTemplate(template);

            return new NullJsonResult();
        }

        [HttpPost]
        public virtual ActionResult DestinationTemplateAdd([Bind(Exclude = "Id")] DestinationTemplateModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();
            ...
            var template = new DestinationTemplate();
            template = model.ToEntity(template);
            _destinationTemplateService.InsertDestinationTemplate(template);

            return new NullJsonResult();
        }

        [HttpPost]
        public virtual ActionResult DestinationTemplateDelete(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();

            var template = _destinationTemplateService.GetDestinationTemplateById(id);
            if (template == null)
                throw new ArgumentException("No template found with the specified id");

            _destinationTemplateService.DeleteDestinationTemplate(template);
