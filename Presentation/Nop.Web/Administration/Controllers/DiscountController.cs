    ...
    public partial class DiscountController : BaseAdminController
    {
        ...
        public virtual ActionResult Edit(DiscountModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (prevDiscountType == DiscountType.AssignedToDestinations
                    && discount.DiscountType != DiscountType.AssignedToDestinations)
                {
                    //applied to destinations
                    discount.AppliedToDestinations.Clear();
                    _discountService.UpdateDiscount(discount);
                }
                if (prevDiscountType == DiscountType.AssignedToProductCodes
        ...
        #region Applied to destinations

        [HttpPost]
        public virtual ActionResult DestinationList(DataSourceRequest command, int discountId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageDiscounts))
                return AccessDeniedKendoGridJson();
            ...
            var destinations = discount
                .AppliedToDestinations
                .Where(x => !x.Deleted)
                .ToList();
            var gridModel = new DataSourceResult
            {
                Data = destinations.Select(x => new DiscountModel.AppliedToDestinationModel
                {
                    DestinationId = x.Id,
