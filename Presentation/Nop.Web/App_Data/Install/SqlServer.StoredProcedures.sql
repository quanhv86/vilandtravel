...
(
	...
	@SearchDestinationPartNumber bit = 0, -- a value indicating whether to search by a specified "keyword" in destination part number
...
BEGIN
	...
	BEGIN
		...
		--destination part number (exact match)
		IF @SearchDestinationPartNumber = 1
		BEGIN
			SET @sql = @sql + '
			UNION
			SELECT p.Id
			FROM Product p with (NOLOCK)
			WHERE p.[DestinationPartNumber] = @OriginalKeywords '
		END
	...
	BEGIN
		SET @sql = @sql + '
		LEFT JOIN Product_Destination_Mapping pmm with (NOLOCK)
	...
	BEGIN
		SET @sql = @sql + '
		AND pmm.DestinationId = ' + CAST(@DestinationId AS nvarchar(max))
	...
	BEGIN
		...
		--destination position (display order)
		IF @DestinationId > 0
		BEGIN
