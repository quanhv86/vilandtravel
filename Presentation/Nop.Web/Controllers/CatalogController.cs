...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        public virtual ActionResult DestinationAll()
        {
            var model = _catalogModelFactory.PrepareDestinationAllModels();
            return View(model);
        }

        [ChildActionOnly]
        public virtual ActionResult DestinationNavigation(int currentDestinationId)
        {
            if (_catalogSettings.DestinationsBlockItemsToDisplay == 0)
                return Content("");

            var model = _catalogModelFactory.PrepareDestinationNavigationModel(currentDestinationId);
